...
glob(PUBLIC_ASSETS_SOURCE_DIRECTORY + '/**/*.*', {}, function(error, files) {
    ...
    async.eachLimit(files, 16, function(file, callback) {
      ...
      function onUploadComplete(error) {
        ...
        del([`${PUBLIC_ASSETS_SOURCE_DIRECTORY}/static`], {force: true}).then(paths => {

          console.log('ls of PUBLIC_ASSETS_SOURCE_DIRECTORY: ', shelljs.ls(PUBLIC_ASSETS_SOURCE_DIRECTORY))
