...
try {
  ...
  var AWS_STATIC_SOURCE_DIRECTORY = getEnvVariable('AWS_STATIC_SOURCE_DIRECTORY','build/static');
...
glob(PUBLIC_ASSETS_SOURCE_DIRECTORY + '/**/*.*', {}, function(error, files) {
    ...
    async.eachLimit(files, 16, function(file, callback) {
      ...
      function onUploadComplete(error) {
        ...
        del([PUBLIC_ASSETS_SOURCE_DIRECTORY], {force: true}).then(paths => {

          console.log('`${BUILD_DIR}/build`: ', `${BUILD_DIR}/build`)
          console.log('ls of BUILD_DIR/build: ', shelljs.ls(`${BUILD_DIR}/build`))
